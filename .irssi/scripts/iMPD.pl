...
my $mpd_help = <<MPD_HELP;
    ...
    %W/mute                      %w- Mutes/Unmutes the volume
    %W/next                      %w- Starts playing next song on playlist
    %W/pause                     %w- Pauses playing
    %W/play <number>             %w- Starts MPD (with optional number of song 
                                              to start on)
...
#    $i =~ s/[.]//g;
    ...
    Irssi::active_win->command( "/say ♫: $i" );
    close Reader;
}
